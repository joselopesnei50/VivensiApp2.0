...
<div id="editor-layout">
    ...
    <div id="left-panel">
        <div class="lp-tabs">
            ...
            <div class="lp-tab" data-tab="icones" onclick="switchTab(this)">Õcones</div>
        ...
        <div id=?tab-icones? class=?lp-content?>
            <div style=?position:relative;padding:8px 0 6px?>
                <i class=?fas fa-magnifying-glass? style=?position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#334155;font-size:11px;pointer-events:none?></i>
                <input type=?text? id=?ico-search? placeholder=?Buscar Ìcone...? oninput=?filterIcons(this.value)?
                    style=?width:100%;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:8px;color:#e2e8f0;font-size:11px;padding:6px 10px 6px 30px;outline:none;transition:border-color .15s?
                    onfocus=?this.style.borderColor=?#6366f1?? onblur=?this.style.borderColor=?rgba(255,255,255,.09)??>
            </div>
            <div class="sec-title">NegÛcios</div>
            <div class="ico-grid" id="ico-negocios"></div>
            <div class="sec-title" style="margin-top:14px">Social & ComunicaÁ„o</div>
            ...
            <div class="sec-title" style="margin-top:14px">SÌmbolos & Decorativos</div>
        ...
        <div id="tab-elementos" class="lp-content">
            ...
            <div class="elem-grid">
                <button class="elem-btn" onclick="addText('heading')"><i class="fas fa-heading"></i>T√≠tulo</button>
                <button class="elem-btn" onclick="addText('sub')"><i class="fas fa-paragraph"></i>Subt√≠tulo</button>
                <button class="elem-btn" onclick="addText('body')"><i class="fas fa-font"></i>Par√°grafo</button>
            ...
            <div class="elem-grid">
                <button class="elem-btn" onclick="addShape('rect')"><i class="fas fa-square"></i>Ret√¢ngulo</button>
                <button class="elem-btn" onclick="addShape('circle')"><i class="fas fa-circle"></i>C√≠rculo</button>
                <button class="elem-btn" onclick="addShape('line')"><i class="fas fa-minus"></i>Linha</button>
                <button class="elem-btn" onclick="addShape('triangle')"><i class="fas fa-play" style="transform:rotate(-90deg)"></i>Tri√¢ngulo</button>
            ...
            <div class="elem-grid">
                <button class="elem-btn" onclick="setBgSolid()"><i class="fas fa-fill-drip"></i>Cor S√≥lida</button>
        ...
        <div id="tab-imagens" class="lp-content">
            ...
            <div id="img-upload" style="display:none">
                <div class="upload-area" onclick="document.getElementById('img-file').click()">
                    ...
                    <p>Clique para enviar<br><span style="font-size:10px;color:#475569">JPG, PNG, WEBP at√© 10MB</span></p>
                </div>
...
<div id="ai-modal" class="modal-overlay">
    <div class="modal-box">
        ...
        <div class="modal-row">
            ...
            <textarea id="ai-prompt" class="modal-input" rows="3" placeholder="Ex: T√≠tulo impactante para campanha de doa√ß√£o de agasalhos..."></textarea>
        </div>
        <div class="modal-row">
            <label class="modal-label">Tipo de texto</label>
            <select id="ai-field" class="modal-input">
                <option value="title">T√≠tulo (Curto)</option>
                <option value="subtitle">Subt√≠tulo (M√©dio)</option>
...
<div id="schedule-modal" class="modal-overlay">
    <div class="modal-box">
        <div class="modal-title"><i class="fas fa-calendar-plus" style="color:#34d399;margin-right:8px"></i>Agendar Publica√ß√£o</div>
...
<div id="tpl-confirm-modal" class="modal-overlay">
    <div class="modal-box" style="width:380px">
        ...
        <p style="color:#94a3b8;font-size:13px;line-height:1.6">Deseja aplicar o template <strong id="tpl-confirm-name" style="color:#e2e8f0"></strong>? O canvas atual ser· substituÌdo.</p>
        <div class="modal-actions">
...
const TEMPLATES = {
    ...
    agency: [
        ...
        { label: 'AgÍncia Square',     previewCss: 'linear-gradient(135deg,#0d1117,#00d4ff)',       build: tplAgenciaSquare },
    ...
    photo: [
        ...
        { label: 'ArrecadaÁ„o (Foto)', preview: 'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=280&q=70', build: tplPhotoNgo },
    ...
    corp: [
        ...
        { label: 'LanÁamento SaaS',    previewCss: 'linear-gradient(135deg,#1e1b4b,#312e81)',       build: tplLancamentoSaas },
    ...
    ngo: [
        ...
        { label: 'Campanha DoaÁ„o',    previewCss: 'linear-gradient(180deg,#ffffff,#ef4444)',       build: tplCampanhaDoacao },
...
const ICONS = {
    negocios: [
        ...
        { label:'Calend√°rio', svg:'<rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><polyline points="9 16 11 18 15 14"/>' },
        { label:'F√°brica', svg:'<path d="M2 20a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V8l-7 5V8l-7 5V4a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v16z"/>' },
        { label:'Sem√°foro', svg:'<rect x="5" y="2" width="14" height="20" rx="2"/><circle cx="12" cy="7" r="2"/><circle cx="12" cy="13" r="2"/><circle cx="12" cy="19" r="2"/><line x1="5" y1="5" x2="2" y2="5"/><line x1="5" y1="11" x2="2" y2="11"/><line x1="19" y1="5" x2="22" y2="5"/><line x1="19" y1="11" x2="22" y2="11"/>' },
        { label:'Escala', svg:'<line x1="12" y1="3" x2="12" y2="21"/><polyline points="17 8 12 3 7 8"/><line x1="3" y1="21" x2="21" y2="21"/>' },
        { label:'R√©gua', svg:'<path d="M4 6h16M4 10h16M4 14h16M4 18h16"/><rect x="2" y="4" width="4" height="16" rx="1"/>' },
        { label:'Gr√°fico Up', svg:'<polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/>' },
        { label:'Trof√©u', svg:'<polyline points="6 9 2 9 2 2 22 2 22 9 18 9"/><path d="M6 2v6c0 3.3 2.7 6 6 6s6-2.7 6-6V2"/><path d="M12 18v4"/><line x1="8" y1="22" x2="16" y2="22"/>' },
    ...
    social: [
        ...
        { label:'Notifica√ß√£o', svg:'<path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/>' },
        { label:'Usu√°rios', svg:'<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>' },
    ...
    deco: [
        ...
        { label:'Cora√ß√£o', svg:'<path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>' },
        { label:'Cora√ß√£o Seta', svg:'<path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78L12 21.23l8.84-8.84a5.5 5.5 0 0 0 0-7.78z"/><line x1="7" y1="17" x2="17" y2="7"/><polyline points="7 7 17 7 17 17"/>' },
        ...
        { label:'Po√ß√£o', svg:'<path d="M10 2v2.343a7.5 7.5 0 1 0 4 0V2"/><line x1="8.5" y1="2" x2="15.5" y2="2"/>' },
...
function addIconToCanvas(svgContent, label) {
    ...
    fabric.Image.fromURL(url, img => {
        ...
        showNotif('√çcone adicionado!', 'success');
    });
}
...
function tplPhotoNgo() {
    ...
    canvas.add(mkTxt('ARRECADA√?√?O\nSOLID√ÅRIA', { sz: Math.round(H*.13), fw:'900', y:90, lh:1.05, shadow: new fabric.Shadow({color:'rgba(0,0,0,.6)',blur:20}) }));
...
function tplPhotoTech() {
    ...
    canvas.add(mkTxt('Automa√ß√£o inteligente para times de alta performance.', { sz: Math.round(H*.033), y: Math.round(H*.6), lh:1.6, w: W*.48, fill:'#cbd5e1' }));
    ...
    canvas.add(mkTxt('Ver Demonstra√ß√£o', { sz:14, fw:'700', x:60, y: Math.round(H*.78)+14, w:210, fill:'#fff', align:'center' }));
    finishTpl();
...
function tplGraphicNeonNgo() {
    ...
    // Subtitle
    canvas.add(mkTxt('Cada contribui√ß√£o molda o futuro que queremos para todos.', { sz: Math.round(H*.034), y: Math.round(H*.66), lh:1.6, fill:'rgba(255,255,255,.85)', name:'sub' }));
    // CTA
...
function tplGraphicImpact() {
    ...
    // Stats pills
    const stats = [{n:'5.200', l:'Fam√≠lias'},{n:'38', l:'Cidades'},{n:'R$1M', l:'Arrecadado'}];
    stats.forEach((s, i) => {
    ...
    canvas.add(mkTxt('‚?ª RELAT√?RIO DE IMPACTO 2025', { sz:11, fw:'800', y:58, fill:'#6ee7b7', name:'tag' }));
    canvas.add(mkTxt('Nosso Impacto\nFala por Si.', { sz: Math.round(H*.12), fw:'900', y:95, lh:1.1, name:'title' }));
    canvas.add(mkTxt('Obrigado a cada volunt√°rio, doador e parceiro.', { sz: Math.round(H*.033), y: Math.round(H*.73), lh:1.5, fill:'rgba(255,255,255,.8)', name:'sub' }));
    finishTpl();
}
...
function tplGraphicAwareness() {
    ...
    canvas.add(mkTxt('#CONSCIENTIZA√?√?O', { sz:11, fw:'800', y:58, fill:'#e879f9', name:'tag' }));
    canvas.add(mkTxt('A Mudan√ßa\nCome√ßa\nem Voc√™.', { sz: Math.round(H*.13), fw:'900', y:95, lh:1.05, shadow: new fabric.Shadow({color:'rgba(236,72,153,.3)',blur:20}), name:'title' }));
...
function tplGraphicMinimal() {
    ...
    canvas.add(mkTxt('Organiza√ß√£o\nNome', { sz: Math.round(H*.05), fw:'800', x:30, y:60, w:200, fill:'#1e293b', align:'center', name:'orgname' }));
    // Main area
    canvas.add(mkTxt('Proposta\nde Valor\nElegante.', { sz: Math.round(H*.1), fw:'900', x: Math.round(W*.42), y:60, w: W*.52, fill:'#0f172a', lh:1.1, name:'title' }));
    canvas.add(mkTxt('Layout clean e profissional para apresenta√ß√µes corporativas e materiais institucionais.', { sz: Math.round(H*.031), x: Math.round(W*.42), y: Math.round(H*.6), w: W*.52, fill:'#475569', lh:1.6, name:'sub' }));
...
function tplGraphicSplit() {
    ...
    canvas.add(tri);
    // Left side content
    canvas.add(mkTxt('NOVO\nLAN√?AMENTO', { sz: Math.round(H*.1), fw:'900', x:40, y:80, w: Math.round(W*.42), fill:'#f8fafc', lh:1.1, name:'title' }));
...
function tplGraphicDuotone() {
    ...
    canvas.add(mkTxt('?è? CONQUISTA 2025', { sz:13, fw:'800', y:55, fill:'#fb923c', name:'tag' }));
    canvas.add(mkTxt('Pr√™mio\nde Excel√™ncia\nEmpresarial.', { sz: Math.round(H*.12), fw:'900', y:95, lh:1.05, shadow: new fabric.Shadow({color:'rgba(251,146,60,.25)',blur:18}), name:'title' }));
    canvas.add(mkTxt('Reconhecimento pela inova√ß√£o e impacto social gerado em 2025.', { sz: Math.round(H*.033), y: Math.round(H*.72), lh:1.6, fill:'rgba(255,255,255,.85)', name:'sub' }));
    finishTpl();
}
...
function tplGraphicBlackFriday() {
    ...
    canvas.add(mkTxt('AT√?', { sz: Math.round(H*.04), fw:'800', x: W*.2, y: Math.round(H*.35), fill:'#f43f5e' }));
    ...
    canvas.add(mkTxt('N√?O PERCA A MAIOR QUEIMA DE ESTOQUE DO ANO', { sz: Math.round(H*.025), fw:'600', x: W*.2, y: Math.round(H*.68), fill:'#a1a1aa' }));
...
function tplCleaningRed() {
    ...
    pts2.forEach((p,i) => canvas.add(mkTxt(`‚?è ${p}`, { sz:12, fw:'600', x:30, y:H*.55+i*26, fill:'#fff', name:'item'+i })));
...
function tplPromoMegaSale() {
    ...
    finishTpl();
    // Produto de fundo ‚?? vai para tr√°s de tudo
...
function tplPromoWeekend() {
    ...
    // Grande mancha azul √† direita (curva org√¢nica superior)
    ...
    // T√≠tulos sobre a mancha azul ‚?? lado direito
...
function tplPromoTravel() {
    ...
    // Ret√¢ngulos de enquadramento decorativos (estilo porta-foto)
...
function tplCorpKpi() {
    ...
    canvas.add(mkTxt('RELAT√?RIO DE DESEMPENHO Q1 2025', { sz:11, fw:'800', y:50, fill:'#475569', name:'tag' }));
    ...
    // KPI row
    const kpis = [{v:'‚?? 34%', l:'Crescimento'},{v:'98%', l:'Satisfa√ß√£o'},{v:'120', l:'Projetos'}];
    kpis.forEach((k, i) => {
        const bx = 60 + i * Math.round((W-120)/3);
    ...
    canvas.add(mkTxt('Meta superada em todos os indicadores. Parab√©ns ao time.', { sz: Math.round(H*.03), y: Math.round(H*.78), fill:'rgba(255,255,255,.5)', lh:1.5, name:'sub' }));
    finishTpl();
}
...
function tplCorpLaunch() {
    ...
    canvas.add(mkTxt('Nossa nova solu√ß√£o transforma a gest√£o de projetos para o pr√≥ximo n√≠vel.', { sz: Math.round(H*.034), y: Math.round(H*.7), lh:1.6, fill:'rgba(255,255,255,.8)', name:'sub' }));
...
function tplCorpWebinar() {
    ...
    canvas.add(mkTxt('‚?è AO VIVO', { sz:10, fw:'800', x:60, y:67, w:80, fill:'#fff', align:'center', name:'live' }));
    ...
    canvas.add(mkTxt('Gest√£o de Alto Impacto em 2025', { sz: Math.round(H*.048), fw:'700', y: Math.round(H*.48), fill:'#38bdf8', name:'sub1' }));
...
function tplBfCamera() {
    ...
    canvas.add(mkTxt('??∑', { sz:32, x:38, y:H-144 }));
...
function tplBusinessOrange() {
    ...
    canvas.add(mkTxt('Transforme\nseu NegÛcio', { sz:Math.round(H*.15), fw:'900', x:30, y:H*.34, fill:'#fff', lh:1.0 }));
...
function tplBfFone() {
    ...
    canvas.add(mkTxt('FONES & √ÅUDIO', { sz:16, fw:'800', x:25, y:65, fill:'rgba(255,255,255,.4)', charSpacing:100 }));
    ...
    canvas.add(mkTxt('??ß', { sz:70, x:W/2-35, y:H/2 }));
...
function tplStartupTalkshow() {
    ...
    canvas.add(mkTxt('Inova√ß√£o &\nNeg√≥cios', { sz:Math.round(H*.14), fw:'900', y:90, fill:'#fff', align:'center', w:W, lh:1.1 }));
    canvas.add(mkCircle({ x:W/2, y:H/2+20, r:45, fill:'rgba(212,175,55,.1)', stroke:'rgba(212,175,55,.3)', strokeWidth:1.5 }));
    canvas.add(mkTxt('???Ô∏è', { sz:40, x:W/2-20, y:H/2-5 }));
    canvas.add(mkRect({ x:60, y:H-100, w:W-120, h:45, fill:'rgba(212,175,55,.15)', stroke:'rgba(212,175,55,.3)', rx:10 }));
    canvas.add(mkTxt('EPIS√?DIO AO VIVO', { sz:14, fw:'700', y:H-85, fill:'#d4af37', align:'center', w:W }));
    finishTpl();
}
...
function tplHeadlineDark() {
    ...
    // Body text
    canvas.add(mkTxt('SubtÌtulo descritivo ou chamada de aÁ„o. Personalize este texto para o seu banner.', { sz:Math.round(H*.033), x:50, y:Math.round(H*.55), w:W*.7, fill:'rgba(255,255,255,.5)', lh:1.7 }));
...
function tplTravelWorld() {
    ...
    // Destinations row
    const dests = ['Paris', 'TÛquio', 'Cairo', 'Lisboa'];
    dests.forEach((d, i) => {
...
function tplFashionPop() {
    ...
    canvas.add(mkTxt('EXPLORAR COLE«√O', { sz:14, fw:'800', y:H-72, fill:'#fff', align:'center', w:W }));
    finishTpl();
}
...
function tplSplitGeo() {
    ...
    canvas.add(mkTxt('SERVI«OS', { sz:10, fw:'700', x:30, y:H*.3, fill:'#a5b4fc', charSpacing:400 }));
    ...
    canvas.add(mkTxt('Sua marca com\nexcelÍncia visual.', { sz:14, color:'rgba(255,255,255,.6)', x:30, y:H*.58, lh:1.5 }));
...
function tplAgenciaSquare() {
    ...
    canvas.add(mkTxt('prÛximo\nnÌvel', { sz:Math.round(H*.12), fw:'900', x:30, y:70+Math.round(H*.12), fill:'#00d4ff', lh:1.1 }));
...
function tplNgoNeon() {
    ...
    canvas.add(mkTxt('‚ù§Ô∏è', { sz:28, x:W/2-21, y:45 }));
    ...
    canvas.add(mkTxt('Transformando vidas atrav√©s da a√ß√£o coletiva', { sz:13, fw:'400', y:H*.58, fill:'rgba(255,255,255,.4)', align:'center', w:W }));
    
    canvas.add(mkRect({ x:60, y:H-90, w:W-120, h:50, fill:'#4f46e5', rx:10 }));
    canvas.add(mkTxt('SEJA VOLUNT√ÅRIO', { sz:16, fw:'800', y:H-72, fill:'#fff', align:'center', w:W }));
    finishTpl();
}
...
function tplCampanhaDoacao() {
    ...
    canvas.add(mkTxt('64% da meta alcan√ßada', { sz:11, fw:'500', x:25, y:295, fill:'#94a3b8' }));
    
    canvas.add(mkRect({ x:25, y:H-80, w:W-50, h:50, fill:'#ef4444', rx:10 }));
    canvas.add(mkTxt('QUERO CONTRIBUIR ‚ù§Ô∏è', { sz:16, fw:'800', x:25, y:H-62, w:W-50, align:'center', fill:'#fff' }));
    finishTpl();
}

function tplVoluntariado() {
    startTpl('linear-gradient(160deg,#1a0a2e 0%,#3b1f5e 50%,#1a0a2e 100%)');
    canvas.add(mkTxt('SEJA VOLUNT√ÅRIO', { sz:12, fw:'700', y:60, fill:'#a855f7', align:'center', w:W, charSpacing:400 }));
    ...
    canvas.add(mkTxt('Fa√ßa parte\nda mudan√ßa', { sz:Math.round(H*.14), fw:'900', y:170, align:'center', w:W, lh:1.1, fill:'#fff' }));
...
function addText(type) {
    const cfgs = {
        heading: { text:'T√≠tulo Principal', sz: Math.round(H*.09), fw:'800', fill:'#111827' },
        sub:     { text:'Subt√≠tulo do Banner', sz: Math.round(H*.04), fw:'600', fill:'#374151' },
...
function addShape(type) {
    ...
    if (type === 'rect')         obj = new fabric.Rect({ left:cx-120, top:cy-80, width:240, height:160, fill:'#6366f1', rx:10, selectable:true, name:'rect' });
        ...
        const group = new fabric.Group([
            ...
            new fabric.Text('PROMO√?√?O', { fontSize:16, fontWeight:'800', fill:'#fff', left:-100, top:-10, width:200, textAlign:'center', fontFamily:'Inter' }),
...
function updateProps() {
    ...
    if (objs.length > 1) {
        ...
        document.getElementById('props-content').innerHTML = `<div class="prop-group"><div class="prop-group-title">Sele√ß√£o M√∫ltipla</div>
            <div class="prop-row">
    ...
    // General actions
    html += `<div class="prop-group"><div class="prop-group-title">A√ß√µes</div>
        <div class="prop-row">
            <button class="prop-btn" onclick="dupActive()"><i class="fas fa-copy"></i> Duplicar</button>
        ...
        <div class="prop-row">
            ...
            <button class="prop-btn" onclick="sendLayer('back')" title="Para Tr√°s"><i class="fas fa-chevron-double-down"></i></button>
        </div>
        <div class="prop-row">
    ...
    if (isText) {
        ...
        html += `<div class="prop-group"><div class="prop-group-title">Tipografia</div>
            ...
            <div class="prop-row">
                ...
                <span class="prop-label" style="margin-left:4px">Espa√ß.</span>
...
function setSaveStatus(s) {
    ...
    if (s==='pending') { el.innerHTML='<i class="fas fa-circle" style="color:#f59e0b;font-size:10px"></i> Altera√ß√µes'; clearTimeout(saveTimer); saveTimer=setTimeout(()=>saveFabric(false),2500); }
...
function saveFabric(showMsg) {
    ...
    } catch(e) { setSaveStatus('pending'); if(showMsg)showNotif('N√£o salvo: imagens externas bloqueiam exporta√ß√£o.','error'); }
}

function exportPng() {
    canvas.discardActiveObject(); 
    canvas.renderAll(); // S√≠ncrono para garantir que n√£o haja camada invis√≠vel
    ...
    setTimeout(() => {
        try {
            ...
            document.body.appendChild(a); // Necess√°rio para Firefox/alguns browsers
            a.click();
...
function runAi() {
    ...
    fetch(AI_URL, { method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':CSRF}, body:JSON.stringify({prompt, field:document.getElementById('ai-field').value}) })
        ...
        .catch(()=>{ btn.disabled=false; btn.innerHTML='<i class="fas fa-wand-magic-sparkles"></i> Gerar'; showNotif('Falha de conex√£o.','error'); });
}

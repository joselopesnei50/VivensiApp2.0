...
<div class="vivensi-card" style="max-width: 800px; margin: 0 auto; border: 1px solid #f1f5f9; background: white; padding: 45px; border-radius: 28px; box-shadow: 0 20px 50px rgba(0,0,0,0.02);">
    ...
    <form action="{{ $basePath . '/projects/'.$project->id }}" method="POST">
        @csrf
        @method('PUT')
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control-vivensi" 
                            ...
                            <option value="canceled" {{ $project->status == 'canceled' ? 'selected' : '' }}>⛔ Cancelado</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div style="display: flex; gap: 20px; align-items: center; padding-top: 20px; border-top: 1px solid #f1f5f9;">
            ...
            <a href="{{ $basePath . '/projects/details/'.$project->id }}" style="flex: 1; text-align: center; color: #94a3b8; font-weight: 800; font-size: 0.9rem; text-decoration: none; text-transform: uppercase;">Cancelar</a>
        </div>
    </form>
</div>
@endsection

...
@section('content')
<style>
    .ec-badge { display:inline-flex; align-items:center; gap:5px; padding:4px 10px; border-radius:20px; font-size:0.72rem; font-weight:700; }
...
<div class="d-flex justify-content-between align-items-center mb-4 pb-2">
    <div>
        ...
        <p class="text-muted mt-1" style="font-size:0.9rem;">Comunique-se com leads e contatos da sua empresa.</p>
    </div>
    <a href="{{ route('manager.email_campaigns.create') }}"
...
<div class="vivensi-card" style="border-radius:20px; overflow:hidden;">
    @if($campaigns->isEmpty())
        <div style="padding:60px; text-align:center; color:#94a3b8;">
            <div style="font-size:3rem; margin-bottom:16px; opacity:0.4;"><i class="fas fa-envelope-open-text"></i></div>
            <div style="font-weight:800; color:#1e293b; font-size:1.1rem; margin-bottom:8px;">Nenhuma campanha ainda</div>
            <p style="font-size:0.88rem; margin:0 0 24px;">Crie sua primeira campanha para se comunicar com seus leads.</p>
            <a href="{{ route('manager.email_campaigns.create') }}"
    ...
    @else
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f8fafc;" onmouseover="this.style.background='#fcfdfe'" onmouseout="this.style.background=''">
                    <td style="padding:18px 24px; vertical-align:middle;">
                        <a href="{{ route('manager.email_campaigns.show', $c) }}"
                    ...
                    <td style="padding:18px 24px; vertical-align:middle; text-align:center;">
                        @php
                            $badgeClass = match($c->status) { 'draft'=>'ec-badge-draft','sending'=>'ec-badge-sending','sent'=>'ec-badge-sent','error'=>'ec-badge-error','scheduled'=>'ec-badge-scheduled',default=>'ec-badge-draft' };
                            $badgeIcon  = match($c->status) { 'draft'=>'fa-pencil','sending'=>'fa-spinner fa-spin','sent'=>'fa-check','error'=>'fa-triangle-exclamation','scheduled'=>'fa-clock',default=>'fa-pencil' };
                            $badgeLabel = match($c->status) { 'draft'=>'Rascunho','sending'=>'Enviando','sent'=>'Enviada','error'=>'Erro','scheduled'=>'Agendada',default=>ucfirst($c->status) };
                        @endphp
                        <span class="ec-badge {{ $badgeClass }}"><i class="fas {{ $badgeIcon }}"></i> {{ $badgeLabel }}</span>
                    </td>
                    <td style="padding:18px 24px; vertical-align:middle; text-align:center;">
                        <span style="font-weight:700; color:#1e293b; font-size:0.88rem;">{{ $c->recipient_count ? number_format($c->recipient_count) : '—' }}</span>
                    </td>
                    <td style="padding:18px 24px; vertical-align:middle; text-align:center;">
                        <span style="font-size:0.78rem; color:#64748b;">{{ $c->sent_at ? $c->sent_at->format('d/m/Y H:i') : $c->created_at->format('d/m/Y') }}</span>
                    </td>
                    <td style="padding:18px 24px; vertical-align:middle; text-align:right;">
                        <div style="display:flex; gap:6px; justify-content:flex-end; align-items:center;">
                            @if(in_array($c->status, ['draft', 'error']))
                                <form id="_formDisparar{{ $c->id }}" action="{{ route('manager.email_campaigns.send', $c) }}" method="POST" style="display:none;">@csrf</form>
                                <button type="button"
                            ...
                            @endif
                            <a href="{{ route('manager.email_campaigns.show', $c) }}" class="ec-action" title="Ver detalhes">
                                <i class="fas fa-eye" style="font-size:0.78rem;"></i>
                            </a>
                            @if($c->status !== 'sent')
                                <form id="_formExcluir{{ $c->id }}" action="{{ route('manager.email_campaigns.destroy', $c) }}" method="POST" style="display:none;">@csrf @method('DELETE')</form>
                                <button type="button" onclick="abrirModalExcluir({{ $c->id }}, '{{ addslashes($c->name) }}')"
                                        class="ec-action ec-action-red" title="Excluir">
        ...
        @if($campaigns->hasPages())
            <div style="padding:20px 24px; border-top:1px solid #f1f5f9;">{{ $campaigns->links() }}</div>
        @endif
    @endif
</div>
...
<div id="modalDisparar" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center;">
    ...
    <div style="position:relative; background:#fff; border-radius:24px; padding:40px; max-width:460px; width:90%; box-shadow:0 25px 50px rgba(0,0,0,0.15); border:1px solid #e2e8f0;">
        ...
        <p style="text-align:center; color:#64748b; font-size:0.88rem; margin:0 0 28px; line-height:1.6;">Esta ação é <strong>irreversível</strong>. O e-mail será enviado imediatamente.</p>
...
<div id="modalExcluir" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center;">
    ...
    <div style="position:relative; background:#fff; border-radius:24px; padding:40px; max-width:420px; width:90%; box-shadow:0 25px 50px rgba(0,0,0,0.15); border:1px solid #e2e8f0;">
        ...
        <p style="text-align:center; color:#64748b; font-size:0.88rem; margin:0 0 8px; line-height:1.6;">A campanha <strong id="modalENome"></strong> será excluída permanentemente.</p>
...
function abrirModalDisparar(id, nome, publico, destinatarios) {
    ...
    document.getElementById('modalDDestinatarios').textContent = destinatarios ? destinatarios.toLocaleString('pt-BR') + ' contatos (est.)' : 'a calcular no envio';
    document.getElementById('modalDisparar').style.display = 'flex';
    document.body.style.overflow = 'hidden';
}
function fecharModalDisparar() { document.getElementById('modalDisparar').style.display = 'none'; document.body.style.overflow = ''; }
...
function fecharModalExcluir() { document.getElementById('modalExcluir').style.display = 'none'; document.body.style.overflow = ''; }
function confirmarExcluir() { if (_formExcluir) _formExcluir.submit(); }
document.addEventListener('keydown', e => { if (e.key === 'Escape') { fecharModalDisparar(); fecharModalExcluir(); } });
</script>
@endsection

    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; flex-wrap:wrap; gap:16px;">
        ...
        <div style="display:flex; gap:10px; align-items:center;">
            @if(in_array($campaign->status, ['draft', 'error']))
                <form id="formDispararCampanha" action="{{ route('manager.email_campaigns.send', $campaign) }}" method="POST">
                    @csrf
                    <button type="button"
            ...
            @if($campaign->status === 'sent' && $campaign->brevo_campaign_id)
                <form action="{{ route('manager.email_campaigns.stats', $campaign) }}" method="POST">
                    @csrf
                    <button type="submit"
...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        <div class="vivensi-card" style="padding:28px; border-radius:20px; margin-bottom:20px;">
            ...
            @php
                $statusLabel = match($campaign->status) { 'draft'=>'Rascunho','sending'=>'Enviando','sent'=>'Enviada','error'=>'Erro','scheduled'=>'Agendada',default=>ucfirst($campaign->status) };
                $details = [
        ...
        @if(in_array($campaign->status, ['draft', 'error']))
        <div class="vivensi-card" style="padding:20px 28px; border-radius:20px;">
            <form id="formExcluir" action="{{ route('manager.email_campaigns.destroy', $campaign) }}" method="POST" style="display:none;">@csrf @method('DELETE')</form>
            <button type="button" onclick="document.getElementById('formExcluir').submit()"
                    style="width:100%; padding:12px; border:2px solid #fca5a5; border-radius:12px; background:white; color:#ef4444; font-weight:700; font-size:0.85rem; cursor:pointer;">
                <i class="fas fa-trash me-1"></i>Excluir campanha
            </button>
        </div>
        @endif
    </div>
...
<div id="modalDisparar" style="display:none; position:fixed; inset:0; z-index:9999; align-items:center; justify-content:center;">
    ...
    <div style="position:relative; background:#fff; border-radius:24px; padding:40px; max-width:460px; width:90%; box-shadow:0 25px 50px rgba(0,0,0,0.15); border:1px solid #e2e8f0;">
        ...
        <div style="display:flex; gap:12px;">
            ...
            <button onclick="confirmarDisparar()" style="flex:1; padding:14px; border:none; border-radius:12px; background:#6366f1; color:white; font-weight:800; font-size:0.9rem; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px;">

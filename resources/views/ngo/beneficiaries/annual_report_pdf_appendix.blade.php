...
<head>
    <meta charset="utf-8">
    <title>Relatório Social - {{ (int) $year }} (com anexos)</title>
    <style>
        @page { margin: 22px; }
...
<body>
    ...
    <div class="box">
        <h1>Apêndice – Lista de atendimentos (amostra)</h1>
        <div class="muted small">
            Exibindo os {{ number_format((int) $details->count()) }} registros mais recentes do ano.
        </div>
    </div>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width:140px;">Beneficiário</th>
            ...
            <tbody>
                @foreach($details as $d)
                    <tr>
                        ...
                        <td><strong>{{ $d->beneficiary_name }}</strong></td>
                        <td class="center">{{ strtoupper($d->beneficiary_status ?? '—') }}</td>
                ...
                @endforeach
                @if($details->count()===0)
                    <tr><td colspan="6" class="center muted" style="padding:12px;">Sem dados.</td></tr>
                @endif
            </tbody>
        </table>
    </div>

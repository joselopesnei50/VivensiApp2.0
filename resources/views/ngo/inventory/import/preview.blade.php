...
<div class="header-page" style="margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center;">
    ...
    <div style="display: flex; gap: 10px;">
        <a href="{{ route('inventory.import.form') }}" class="btn-premium" style="background:#64748b;">
            <i class="fas fa-times"></i> Cancelar
        </a>
        @if($summary['valid'] > 0)
        <form action="{{ route('inventory.import.confirm') }}" method="POST" style="display: inline;">
            @csrf
...
@if(!empty($parseErrors))
    <div class="alert alert-warning" style="margin-bottom: 20px;">
        ...
        <ul style="margin: 5px 0 0 0;">@foreach($parseErrors as $err)<li>{{ $err }}</li>@endforeach</ul>
    </div>
@endif
    ...
    <table style="width: 100%; border-collapse: collapse; min-width: 900px;">
        <thead style="background: #f8fafc;">
            <tr>
                ...
                <th style="padding: 12px; text-align: left; font-size: 0.75rem; color: #64748b; text-transform: uppercase;">SKU</th>
                <th style="padding: 12px; text-align: left; font-size: 0.75rem; color: #64748b; text-transform: uppercase;">Unidade</th>
                <th style="padding: 12px; text-align: right; font-size: 0.75rem; color: #64748b; text-transform: uppercase;">Qtd</th>
                <th style="padding: 12px; text-align: right; font-size: 0.75rem; color: #64748b; text-transform: uppercase;">Mín</th>
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid #f1f5f9; {{ $row['_error'] ? 'background: #fef2f2;' : '' }}">
                ...
                <td style="padding: 10px; color: #64748b;">{{ $row['sku'] ?: '—' }}</td>
                <td style="padding: 10px; color: #64748b;">{{ $row['unidade'] ?? '—' }}</td>
                <td style="padding: 10px; text-align: right; color: #334155;">
                    {{ is_numeric($row['quantidade'] ?? null) ? number_format($row['quantidade'], 2, ',', '.') : '—' }}
                </td>
                <td style="padding: 10px; text-align: right; color: #64748b;">
                    {{ is_numeric($row['estoque_minimo'] ?? null) ? number_format($row['estoque_minimo'], 2, ',', '.') : '—' }}
                </td>

    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; flex-wrap:wrap; gap:16px;">
        ...
        <div style="display:flex; gap:10px; align-items:center;">
            @if(in_array($campaign->status, ['draft', 'error']))
                <form id="formDispararCampanha" action="{{ route('ngo.email_campaigns.send', $campaign) }}" method="POST">
                    @csrf
                    <button type="button"
            ...
            @if($campaign->status === 'sent' && $campaign->brevo_campaign_id)
                <form action="{{ route('ngo.email_campaigns.stats', $campaign) }}" method="POST">
                    @csrf
                    <button type="submit"
...
<div class="row g-4 mb-4">
    @php
        $metrics = [
            ...
            ['label'=>'Cliques',       'value'=>$campaign->clickRate() !== null ? $campaign->clickRate().'%' : '—',
             'sub' =>$campaign->stat_clicks ? number_format($campaign->stat_clicks).' únicos' : null,                                                             'icon'=>'fa-arrow-pointer',  'color'=>'#3b82f6', 'bg'=>'#eff6ff'],
...
<div class="row g-4">
    ...
    <div class="col-lg-4">
        ...
        {{-- Excluir (somente rascunho/erro) --}}
        @if(in_array($campaign->status, ['draft', 'error']))
        <div class="vivensi-card" style="padding:20px 28px; border-radius:20px;">
            <form id="formExcluir" action="{{ route('ngo.email_campaigns.destroy', $campaign) }}" method="POST" style="display:none;">@csrf @method('DELETE')</form>
            <button type="button"
...
function abrirModalDisparar(nome, publico, destinatarios) {
    ...
    document.getElementById('modalDestinatarios').textContent = destinatarios
        ? destinatarios.toLocaleString('pt-BR') + ' contatos'
        : 'a calcular no envio';

...
class MetaSocialInsightsService
{
    ...
    private function syncFacebook(ScheduledPost $post, string $token): bool
    {
        ...
        // salvaram só photo_id/objectId (sem prefix). Formato correto pra
        // Insights: PAGEID_OBJECTID. Constroi manualmente usando o page_id
        // da conta vinculada. Salvamos no DB pra próximas execuções.
        $fbId = $post->facebook_post_id;
        $pageId = $post->account?->page_id;

        if ($fbId && $pageId && !str_contains($fbId, '_')) {
            $fixed = "{$pageId}_{$fbId}";
            Log::info('FB post_id backfill (construído)', [
                'post_id' => $post->id, 'antes' => $fbId, 'depois' => $fixed,
            ]);

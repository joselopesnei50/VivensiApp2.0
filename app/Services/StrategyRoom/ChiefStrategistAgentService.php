...
class ChiefStrategistAgentService
{
    ...
    public function synthesize(int $sessionId): array
    {
        ...
        $priorMessages = StrategyMessage::withoutGlobalScopes()
            ...
            ->whereIn('agent', ['financeiro', 'inteligencia'])
            ->orderBy('created_at', 'asc')
            ->get(['agent', 'content', 'confidence']);

        if ($priorMessages->isEmpty()) {
            return [
                'error'      => 'Nao ha falas de Financeiro/Inteligencia pra sintetizar.',
...
- fatos_usados: array com os agentes cuja fala voce efetivamente usou na sintese. Valores validos: "financeiro" e "inteligencia" (nao inclua outros).

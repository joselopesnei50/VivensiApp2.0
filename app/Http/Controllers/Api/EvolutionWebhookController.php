...
class EvolutionWebhookController extends Controller
{
    public function handle(Request $request, string $token)
    {
        ...
        //    withoutGlobalScopes: rota pública (sem auth) — o scope fail-closed
        //    bloquearia o lookup; o bidx do token já isola o tenant.
        $instance = WhatsappInstance::withoutGlobalScope('tenant')
            ->where('instance_token_bidx', hash_hmac('sha256', $token, whatsapp_bidx_key()))
            ->first();
